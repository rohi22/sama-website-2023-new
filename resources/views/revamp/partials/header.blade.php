@php
    $machines = DB::table('categories')
        ->where('parent_id', '=', 3)
        ->where('status', 1)
        ->orderBy('cat_order', 'ASC')
        ->get(['categories.*']);
    $processingSeries = DB::table('categories')
        ->where('parent_id', '=', 79)
        ->where('status', 1)
        ->orderBy('cat_order', 'ASC')
        ->get(['categories.*']);
    $bakerySeries = DB::table('categories')
        ->where('parent_id', '=', 80)
        ->where('status', 1)
        ->orderBy('cat_order', 'ASC')
        ->get(['categories.*']);
    $pharmaLine = DB::table('categories')
        ->where('parent_id', '=', 111)
        ->where('status', 1)
        ->orderBy('cat_order', 'ASC')
        ->get(['categories.*']);
                        ...
                        <ul class="THLinks">
                            ...
                            <li>
                                <div class="input-group" style="display: flex; align-items: center;">
                                    <form action="{{ route('revamp.search') }}" method="get">
                                        @csrf
                                        <input type="text"
                ...
                <div class="col-lg-12 d-flex align-items-center justify-content-lg-between justify-content-center">
                    ...
                    <nav class="sam-nav">
                        <ul class="d-flex flex-column flex-lg-row nav-col">
                            <li>
                                <a href="{{ asset('revamp/category/machines') }}">Machines <i
                            ...
                            <li>
                                <a href="{{ asset('revamp/category/processing-series') }}">Processing Line <i
                                        class="fa fa-chevron-down ms-2"></i></a>
                                <div class="SUBLinks">
                                    <div class="container p-0">
                                        <ul class="menu-carousel owl-carousel">
                                            @forelse($processingSeries as $processing)
                                                <li>
                                                    <a
                                                        href="{{ asset('revamp/sub-category/' . $processing->cat_slug) }}">
                                                        <span><img
                                                                src="{{ asset('uploads/' . $processing->cat_icon) }}" /></span>
                            ...
                            <li>
                                <a href="{{ asset('revamp/category/bakery-series') }}">Bakery Series <i
                                        class="fa fa-chevron-down ms-2"></i></a>
                                <div class="SUBLinks">
                                    <div class="container p-0">
                                        <ul class="menu-carousel owl-carousel">
                                            @forelse($bakerySeries as $bakery)
                                                <li>
                                                    <a href="{{ asset('revamp/sub-category/' . $bakery->cat_slug) }}">
                                                        <span><img
                                                                src="{{ asset('uploads/' . $bakery->cat_icon) }}" /></span>
                            ...
                            <li>
                                <a href="{{ asset('revamp/category/pharmaceutical-line') }}">Pharmaceutical Line <i
                                        class="fa fa-chevron-down ms-2"></i></a>
                                <div class="SUBLinks">
                                    <div class="container p-0">
                                        <ul class="menu-carousel owl-carousel">
                                            @forelse($pharmaLine as $pharma)
                                                <li>
                                                    <a href="{{ asset('revamp/sub-category/' . $pharma->cat_slug) }}">
                                                        <span><img
                                                                src="{{ asset('uploads/' . $pharma->cat_icon) }}" /></span>
                            ...
                            <li><a href="{{ asset('revamp/category/snack-processing') }}">Snack Processing Line</a>
                            </li>
    ...
    <div class="mobo-menu container-fluid headereffect customheight">
        <div class="sama-header-row">
                            ...
                            @forelse($categories as $parent)
                                        ...
                                        <ul>
                                            @forelse($parent->subcategories as $child)
                                                <li class="pushy-link suby" style="font-size: 12px;"><button><a
                                                            href="{{ asset('sub-category/' . $child->cat_slug) }}">{{ $child->cat_title }}</a></button>
                                                </li>
                                            @empty
                                            @endforelse
                                        </ul>
                                ...
                                @else
                                    <li class="pushy-link"><button><a
                                                href="{{ asset('category/' . $parent->cat_slug) }}">{{ $parent->cat_title }}</a></button>
                                    </li>
                                @endif
                            @empty
                            @endforelse
            ...
            <div class="col-8 logo-col">
                <div class="logo">
                    <a href="{{ url('/revamp') }}"><img
                            src="https://www.samaengineering.com/uploads/logos/logo-mobile.jpeg" alt=""
